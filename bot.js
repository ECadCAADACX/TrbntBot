...
//**=play <name> / To Play The Son=stop / To Stop The Song=skip / To Skip The Song=vol <number> / To Change Volume
...
client.on('message', async msg => { // eslint-disable-line
	...
	if (command === `play`) {
		...
		if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
			...
			return msg.channel.send(` **${playlist.title}** The Song In Waiting List!`);
		} else {
			try {
...
${videos.map(video2 => `[**${++index} **] \`${video2.title}\``).join('\n')}`)
					...
					try {
						...
						console.error(err);
						return msg.channel.send('You Have To Choose Song First.');
					}
					const videoIndex = parseInt(response.first().content);
					var video = await youtube.getVideoByID(videos[videoIndex - 1].id);
				} catch (err) {
					console.error(err);
					return msg.channel.send(':pensive:  I Cant Find Nothing.');
				}
			}
...
**Song Starting** ${serverQueue.songs[0].title}`)
...
//	console.log('yao: ' + Util.escapeMarkdown(video.thumbnailUrl));
	...
	if (!serverQueue) {
		...
		else return msg.channel.send(` **${song.title}** The Song Is In Waiting List!`);
	}
	return undefined;
}
...
if (message.content.startsWith(prefix + 'setstream')) {
  client.user.setGame(argresult, "https://www.twitch.tv/omar_9897");
...
client.login("NDg4NjUxODgyMDAxMDcyMTI5.Dn5lJg.ogJ_xGfAX7jEG8AiAkQksxnrmAE");
