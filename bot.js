...
client.on('message', async msg =>{
	if (msg.author.bot) return udefined;
...
client.on('message', async msg => { 
	...
	if (commands === `play`,`p`) {
		...
		if (!permissions.has('EMBED_LINKS')) {
			...
			return msg.channel.sendMessage(":x: **You have to be in a voice channel to use this command.**")
		}
		...
		if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
			...
			return msg.channel.send(`:heavy_check_mark: **Searching**:mag_right: **``${playlist.title}``** `);
		} else {
...
**Now playing :** **${serverQueue.songs[0].title}**`)
		...
		if (serverQueue && serverQueue.playing) {
			...
			return msg.channel.send(':stopwatch: ***paused*** :thumbsup: ');
		}
...
client.on('message', message => {
    if (message.content === '@TrbntBot#2732') {
        let helpEmbed = new Discord.RichEmbed()
        .setTitle('**My prefix here is** **``=``**')
